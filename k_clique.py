...
class GN(object):
    ...
    def execute(self):
        for k in range(3, 6):
            components = [list(c) for c in list(nx.algorithms.community.k_clique_communities(G, k))]
...
if __name__ == '__main__':
    ...
    nx.draw(G, pos=pos, nodelist=partition[0], node_color='g', node_shape='D')
    nx.draw(G, pos=pos, nodelist=partition[1], node_color='b', node_shape='o')
    nx.draw(G, pos=pos, nodelist=partition[2], node_color='r', node_shape='v')
    plt.show()
